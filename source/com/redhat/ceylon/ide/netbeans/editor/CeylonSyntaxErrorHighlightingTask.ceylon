                ...
                for (error in CeylonIterable(that.errors)) {
                    ...
                    if (exists _s = start,
                        ...
                        if (is RecognitionError error,
                         ...
                         value fileObject = NbEditorUtilities.getFileObject(document);
                         value fixes = ArrayList<Fix>();
                         value data = NbQuickFixData(controller.lastCompilationUnit, controller.project,
                             that, error, document, fixes);
                         
                         nbQuickFixManager.addQuickFixes(data, controller.typeChecker, fileObject);
